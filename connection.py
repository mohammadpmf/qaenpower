...
class Connection():
    def __init__(self, host='127.0.0.1', username='root', password='root', db='amar'):
        ...
        queries = [
            ...
            "CREATE TABLE IF NOT EXISTS `tbl_parameters_log` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `value` DECIMAL(20,10) NOT NULL, `workout` DECIMAL(20,10) NOT NULL, `is_ok` TINYINT(1) NOT NULL DEFAULT 1, `date` DATE NOT NULL, `date_time_modified` DATETIME NOT NULL, `parameter_id` INT UNSIGNED NOT NULL, `user_id` INT UNSIGNED NOT NULL, `failure_reason` VARCHAR(63) NOT NULL DEFAULT '', PRIMARY KEY (`id`), INDEX `date_index` (`date` ASC), CONSTRAINT `parameter_id` FOREIGN KEY (`parameter_id`) REFERENCES `tbl_parameters` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE, CONSTRAINT `user_id` FOREIGN KEY (`user_id`) REFERENCES `tbl_users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE) ENGINE = MyISAM;",
