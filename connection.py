...
class Connection():
    def __init__(self, host='127.0.0.1', username='root', password='root', db='amar'):
        ...
        queries = [
            "CREATE TABLE IF NOT EXISTS `tbl_users` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `name` VARCHAR(64) NOT NULL, `surname` VARCHAR(64) NOT NULL, `username` VARCHAR(64) NOT NULL, `password` VARCHAR(128) NOT NULL, `access_level` TINYINT(1) NOT NULL DEFAULT 2, `wrong_times` TINYINT(2) NOT NULL DEFAULT 0, `default_date` VARCHAR(64) NOT NULL DEFAULT 'روز قبل', PRIMARY KEY (`id`), UNIQUE INDEX `username_UNIQUE` (`username` ASC)) ENGINE = MyISAM;",
            "CREATE TABLE IF NOT EXISTS `tbl_sections` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `title` VARCHAR(45) NOT NULL, `order` INT UNSIGNED NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `title_UNIQUE` (`title` ASC)) ENGINE = MyISAM;",
            ...
            "CREATE TABLE IF NOT EXISTS `tbl_parameters` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `name` VARCHAR(45) NOT NULL, `type` VARCHAR(45) NOT NULL, `unit` VARCHAR(45) NULL, `default_value` VARCHAR(45) NOT NULL, `variable_name` VARCHAR(45) NOT NULL, `warning_lower_bound` DECIMAL(20,10) NULL, `warning_upper_bound` DECIMAL(20,10) NULL, `alarm_lower_bound` DECIMAL(20,10) NULL, `alarm_upper_bound` DECIMAL(20,10) NULL, `formula` VARCHAR(255) NOT NULL DEFAULT '', `section` INT UNSIGNED NOT NULL, `place` INT UNSIGNED NOT NULL, `order` INT UNSIGNED NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `variable_name_UNIQUE` (`variable_name` ASC) , UNIQUE INDEX `parameter_place_section` (`name` ASC, `place` ASC, `section` ASC) , INDEX `section2_idx` (`section` ASC) , INDEX `place_idx` (`place` ASC) , CONSTRAINT `section2` FOREIGN KEY (`section`) REFERENCES `tbl_sections` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT, CONSTRAINT `place` FOREIGN KEY (`place`) REFERENCES `tbl_places` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT) ENGINE = MyISAM;",
            "CREATE TABLE IF NOT EXISTS `tbl_parameters_log` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `value` DECIMAL(20,10) NOT NULL, `workout` DECIMAL(20,10) NOT NULL, `is_ok` TINYINT(1) NOT NULL DEFAULT 1, `date` DATE NOT NULL, `date_time_modified` DATETIME NOT NULL, `parameter_id` INT UNSIGNED NOT NULL, `user_id` INT UNSIGNED NOT NULL, `failure_reason` VARCHAR(255) NOT NULL DEFAULT '', PRIMARY KEY (`id`), INDEX `date_index` (`date` ASC), CONSTRAINT `parameter_id` FOREIGN KEY (`parameter_id`) REFERENCES `tbl_parameters` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE, CONSTRAINT `user_id` FOREIGN KEY (`user_id`) REFERENCES `tbl_users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE) ENGINE = MyISAM;",
            "CREATE TABLE IF NOT EXISTS `tbl_parameters_real_log` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `value` DECIMAL(20,10) NOT NULL, `workout` DECIMAL(20,10) NOT NULL, `is_ok` TINYINT(1) NOT NULL DEFAULT 1, `date` DATE NOT NULL, `date_time_modified` DATETIME NOT NULL, `parameter_id` INT UNSIGNED NOT NULL, `user_id` INT UNSIGNED NOT NULL, `failure_reason` VARCHAR(255) NOT NULL DEFAULT '', PRIMARY KEY (`id`), INDEX `date_index` (`date` ASC), CONSTRAINT `parameter_id` FOREIGN KEY (`parameter_id`) REFERENCES `tbl_parameters` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE, CONSTRAINT `user_id` FOREIGN KEY (`user_id`) REFERENCES `tbl_users` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE) ENGINE = MyISAM;",
            "CREATE TABLE IF NOT EXISTS `tbl_parameters_max_date` (`id` INT NOT NULL AUTO_INCREMENT, `parameter_id` INT UNSIGNED NOT NULL, `max_date` DATE NOT NULL, PRIMARY KEY (`id`)) ENGINE = MEMORY;",
            "CREATE TABLE IF NOT EXISTS `tbl_parameters_min_date` (`id` INT NOT NULL AUTO_INCREMENT, `parameter_id` INT UNSIGNED NOT NULL, `min_date` DATE NOT NULL, PRIMARY KEY (`id`)) ENGINE = MEMORY;",
        ]
